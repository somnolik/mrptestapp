...
nlp="http://www.digital-archiv.at/ns/mrptestapp/nlp" at "../modules/nlp.xqm";
import module namespace util = "http://exist-db.org/xquery/util";
import module namespace httpclient ="http://exist-db.org/xquery/httpclient";
declare namespace tei="http://www.tei-c.org/ns/1.0";

let $collection := '/db/apps/mrptestapp/data/editions/'
let $input := doc('/db/apps/mrptestapp/data/editions/rollet.xml')
let $profile := 'default'
let $language := 'de'
...
(: let $tokInput := doc('/db/apps/mrptestapp/nlp/temp/rollet.xml'):)
(: let $pos := nlp:pos-tagging($tokInput):)
(: return $pos:)
 
  (: example for nlp:pos-tagging-post :)
(: let $tokInput := doc('/db/apps/mrptestapp/nlp/temp/bezirkskommissariate-an-stadthauptmannschaft-1851-03-a3-xxi-d109.xml'):)
